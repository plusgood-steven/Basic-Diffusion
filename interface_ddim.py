...
def sample_data(model, batch_size, sample_num, ddim_timesteps, dir_path, device):
    ...
    while sample_num:
        ...
        images = model.sample(
            one_sample_batch, ddim_timesteps=ddim_timesteps, device=device)
...
def sample_grid(model, sample_num, ddim_timesteps, dir_path, device):
    ...
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)
    images = model.sample_grid(
        sample_num, 8, device, ddim_timesteps=ddim_timesteps)
...
if __name__ == "__main__":
    ...
    diffusionModel = DDIM(model_UNet, image_size=(
        28, 28), img_channels=3, betas=beta).to(device)
    diffusionModel.load_state_dict(checkpoint['model'])

    model_parameters = count_parameters(diffusionModel)

    print(f'model parameters: {model_parameters}')
    if not params.only_sample_grid:
        sample_data(diffusionModel, args.sample_size, args.sample_data_num,
                    args.ddim_timesteps, args.dir_path, device)
    sample_grid(diffusionModel, 8, args.ddim_timesteps, args.dir_path, device)
