...
def train(train_loader, model, args, device):
    ...
    for epoch in range(n_epochs):
        ...
        for x in train_pbar:
            ...
            train_pbar.set_postfix({'loss': loss.detach().item(), 'lr': optimizer.param_groups[0]['lr']})
        ...
        if (epoch+ 1) % args.sample_num == 0:
            FID = sample_data(model, args.sample_size, args.sample_data_num, dir_path=args.dir_path, fid_path=args.fid_path, device=device)
...
def sample_data(model, batch_size, sample_num, dir_path, fid_path, device):
    ...
    while sample_num > 0:
        ...
        all_images = torch.cat((all_images, images), dim=0) if all_images else images
    ...
    all_images = all_images.mul(255).add_(0.5).clamp_(0, 255).permute(0, 2, 3, 1).to("cpu", torch.uint8).numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--channel_mults",default=(1, 2, 2), type=int, nargs='+')
    ...
    defussionModel = GaussianDiffusion(model_UNet, image_size=(28,28), img_channels=3, betas=beta).to(device)

    model_parameters = count_parameters(defussionModel)
    logging.info(f'args: {args}')
    logging.info(f'model parameters: {model_parameters}')
    logging.info(f'model: {defussionModel}')
    print(f'model parameters: {model_parameters}')

    train(train_loader, defussionModel, args, device)
